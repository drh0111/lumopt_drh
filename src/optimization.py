...
class Optimization(SuperOptimization):
    ...
    SRC_NAMES:   List class. It contains the name of sources
    ...
    ADJOINT_FIELDS:
    """
    def __init__(self, base_script, fom, geometry, optimizer, wavelength, src_names = ['source'], use_var_fdtd = False,  hide_fdtd_cad = False, dir_grad = True, plot_hist = True, store_all = True) -> None:
    ...
    def initialize(self):
        ...
        for src_name in self.src_names:
            Optimization.set_source_wavelength(self.sim, src_name, self.fom.mul_freq_src, len(self.wavelength))
    ...
    def make_forward_sim(self, params):
        ...
        for src_name in self.src_names: # Used to set forward source
            self.sim.fdtd.setnamed(src_name, 'enabled', True)
    ...
    def run_forward_solver(self, params):
        ...
        fom, sp_component = self.fom.get_fom(self.sim) # For PolarMatch class, be aware to add sp_component
        if self.store_all:
            self.sim.save()
    ...
    def make_adjoint_sim(self, params):
        ...
        self.geometry.add_geo(self.sim, params = None, only_update = True)
        for src_name in self.src_names:
            self.sim.fdtd.setnamed(src_name, 'enabled', False)
    ...
    def run_adjoint_solves(self, params):
        ...
        if not has_forward_field or params_changed:
    ...
    def set_source_wavelength(sim, source_name, mul_freq_src, freq_pts):
        ...
        if sim.fdtd.getnamed(source_name, 'override global source settings'):
            print('The wavelength setting of the source will be superseded by the global settings')
