...
def get_eps_from_sim(fdtd, monitor_name = 'opt_fields', unfold_symmetry = True):
    """
    This function is used to get the data of epsilon, x, y, z, and lambda from the monitor in Lumerical FDTD
    """

    index_mon_name = monitor_name + '_index'
    ...
    ).format(unfold_symmetry_str, index_mon_name))

    fields_eps_x = fdtd.getv('{0}_eps_x'.format(index_mon_name))
    fields_eps_y = fdtd.getv('{0}_eps_y'.format(index_mon_name))
    fields_eps_z = fdtd.getv('{0}_eps_z'.format(index_mon_name))
    index_mon_x = fdtd.getv('{0}_x'.format(index_mon_name))
    index_mon_y = fdtd.getv('{0}_y'.format(index_mon_name))
    index_mon_z = fdtd.getv('{0}_z'.format(index_mon_name))
    index_mon_lamda = fdtd.getv('{0}_lambda'.format(index_mon_name))

    fields_eps = np.stack((fields_eps_x, fields_eps_y, fields_eps_z), axis = -1)
    return fields_eps, index_mon_x, index_mon_y, index_mon_z, index_mon_lamda