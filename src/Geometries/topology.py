...
class Density2D(Geometry):
    ...
    def __init__(self, eps, x, y, z, eps_min, eps_max):
        ...
        X_SPAN: Real value
            X span of the structure unit cell
        Y_SPAN: Real value
            Y span of the structure unit cell
        DEPTH:  Real value
    ...
    def add_geo(self, sim, params, only_update):
        ...
        if not only_update:
            # Delete the original Lumerical defined geometry
            script = ('select("rect_pc::circle");'
                      'delete;'
                      'delete;') # The name should follow specific setting. Probelm: better way to settle it? 2 Delete just work, but don't know why yet
            ...
            ## Only add mesh when necessary (like you specify list x by yourself), or it will cost extra time, and be aware that it is uniform mesh
            # mesh_script = ('addmesh;'
            #                'set("x min", {});'
            #                'set("x max", {});'
            #                'set("y min", {});'
            ...
            #                'set("z max", {});'
            #                'set("dx", {});'
            #                'set("dy", {});').format(np.amin(self.x), np.amax(self.x), np.amin(self.y), np.amax(self.y), self.z-self.depth/2, self.z+self.depth/2, self.dx, self.dy)
        ...
        if eps is not None:
            ...
            ## This version is for simulation of periodic structure, so we will add 9 unit cells to form the whole structure
            script = ('select("import");'
                       'delete;'
                       'tmp = zeros(length(x_geo), length(y_geo), 2);'
                       'tmp(:, :, 1) = eps_geo;'
            ...
            for i_x in range(3):
                for i_y in range(3):
                    script = ('addimport;'
                              ...
                              'set("x", {});'
                              'set("y", {});'
                    ).format((i_x-1)*self.x_span, (i_y-1)*self.y_span)
                    sim.fdtd.eval(script)
    ...
    def calculate_gradients(self, gradient_fields):
        ...
        d_eps = np.atleast_3d(self.from_density_to_eps(np.reshape(d_param, self.size)) - self.eps)
        gradients = np.reshape(d_eps*dF_dEps, (d_eps.size, -1)) / dx
